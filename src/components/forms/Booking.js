        ...
        if(room.quantity > 0){
            return(
                <div className="col-12 col-lg-6 pb-5" key={room._id}>
                    <div className="card" >
                    <img style={{"maxWidth": "700px", "width": "100%"}} className="card-img-top img-fluid" src={`${URL}${room.images[0].path}`} />
                        <div className="card-body">
                            
                            <Heading.H3 location="booking">{room.name}</Heading.H3>
                            <Heading.H4 location="booking">Available rooms: {room.quantity}</Heading.H4>
                            <Heading.H4 location="booking">P {room.price}</Heading.H4>
                            
                            <InteractionDiv>
                                <input style={{"width":"20%", "float": "left"}}type="number" name="quantity" min="1" max={room.quantity} />
                                <Button size="large"style={{"width": "80%"}}location="booking" onClick={(e) => {
            ...
            <BookingContent className="p-5">
                ...
                <Fragment>
                    ...
                    {booking && availableRooms.length > 0 ? 
                        <CatalogContainer>
                            <Heading.H1 location="checkout" className="text-center">Accommodations</Heading.H1>
                            <hr />
                            <div className="row">
                                {showAvailableRooms}
                            </div>
                            <div className="text-right " style={{"size": "5rem"}}>
                                <Link to="/checkout" ><Button location="booking"  size="large">Checkout</Button></Link>
                            </div>
